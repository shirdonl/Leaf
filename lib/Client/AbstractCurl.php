...
use Leaf\Configuration\ParameterBag;
use Leaf\Exception\CallbackException;
use Leaf\Exception\ClientException;
use Leaf\Exception\NetworkException;
use Leaf\Exception\RequestException;
use Leaf\Message\HeaderConverter;
use Leaf\Message\ResponseBuilder;
...
abstract class AbstractCurl extends AbstractClient
{
    ...
    private function setOptionsFromRequest($curl, RequestInterface $request): void
    {
        $options = [
            ...
            CURLOPT_HTTPHEADER => HeaderConverter::toLeafHeaders($request->getHeaders()),
        ];
